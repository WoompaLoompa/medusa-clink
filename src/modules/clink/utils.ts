...
export class CurrencyService {
  ...
  constructor(options: ClinkOptions) {
    this.source = options.currencySource || CLINK_DEFAULTS.currencySource!
   ...
   */
  async fiatToSats(amount: number, currency: string): Promise<ConversionResult> {
   ...
   */
  private async fetchCoinGeckoRate(currency: string): Promise<number> {
   ...
   */
  private async fetchKrakenRate(currency: string): Promise<number> {
   ...
   */
  getCacheStatus(): Record<string, { rate: number; age: number }> {
    const status: Record<string, { rate: number; age: number }> = {}
 ...
 */
export function extractOfferPrefix(noffer: string): string {
...
export function formatSats(
  sats: number,
  format: "sats" | "btc" | "btc-symbol" = "sats"
): string {
  switch (format) {
    case "btc":
