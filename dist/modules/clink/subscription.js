...
class SubscriptionService {
    ...
    async processPayment(subscriptionId) {
        ...
        try {
            ...
            const response = await this.clinkSDK_.Debit(debitRequest);
            // Handle response
    ...
    async getCustomerSubscriptions(customerId) {
        return Array.from(subscriptionStore.values()).filter(sub => sub.customer_id === customerId);
    }
    /**
     * Get subscriptions due for payment
     */
    async getSubscriptionsDueForPayment() {
        const now = Math.floor(Date.now() / 1000);
        return Array.from(subscriptionStore.values()).filter(sub => sub.status === "active" && sub.current_period_end <= now);
    }
    /**
